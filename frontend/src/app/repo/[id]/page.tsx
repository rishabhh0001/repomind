...
export default function RepoWorkspace() {
  ...
  if (loading || (repo && repo.status !== "ready" && repo.status !== "error")) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center space-y-4 bg-[#0c0c0c] text-white">
        <MaterialIcon name="sync" size={36} className="animate-spin text-indigo-400" />
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0c0c0c] overflow-hidden select-none">
      {/* Sub Navbar */}
      <div className="h-12 border-b border-white/5 bg-[#111111] shrink-0 flex items-center px-4 justify-between z-20">
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="w-full md:w-[420px] h-[45vh] md:h-full shrink-0 border-b md:border-b-0 md:border-r border-white/5 bg-[#111111] flex flex-col z-10 overflow-hidden">
          <div className="flex items-center p-2 border-b border-white/5 gap-1 shrink-0 bg-[#111111]">
            ...
            >
              <MaterialIcon name="chat_bubble" size={15} /> <span>Chat</span>
            </button>
            <button
        ...
        {/* Middle: Graph Canvas */}
        <div className="flex-1 min-h-0 h-full relative bg-[#0c0c0c] overflow-hidden">
          <CodeGraph repoId={repoId} />
        </div>
        
        {/* Right sidebar: Inspector */}
        {isInspectorOpen && (
          <div className="w-full md:w-[500px] shrink-0 border-t md:border-t-0 md:border-l border-white/5 bg-[#111111]/95 backdrop-blur-xl shadow-2xl z-20 absolute md:right-0 bottom-0 md:top-0 h-[50vh] md:h-full overflow-hidden flex flex-col">
            <NodeInspector />
          </div>
        )}
      </div>
